...
static const char *find_linux_crt(void)
{
	...
	if (compiler.platform.arch == ARCH_TYPE_RISCV64 || compiler.platform.arch == ARCH_TYPE_RISCV32)
	{
		...
		if (path)
		{
			INFO_LOG("Found crt at %s (sysroot)", path);
			return path;
		}
		...
		if (path)
		{
			INFO_LOG("Found crt at %s (std)", path);
			return path;
		}
	}
	else
	...
	// Fallback for native linking (e.g. Alpine)
	bool is_host_arch = false;
#if defined(__x86_64__) || defined(_M_X64)
	is_host_arch = compiler.platform.arch == ARCH_TYPE_X86_64;
#elif defined(__i386__) || defined(_M_IX86)
	is_host_arch = compiler.platform.arch == ARCH_TYPE_X86;
#elif defined(__aarch64__) || defined(_M_ARM64)
	is_host_arch = compiler.platform.arch == ARCH_TYPE_AARCH64;
#endif

	if (is_host_arch)
	{
		if (file_exists("/usr/lib/crt1.o")) return "/usr/lib";
		if (file_exists("/usr/lib64/crt1.o")) return "/usr/lib64";
		if (file_exists("/lib/crt1.o")) return "/lib";
	}
...
static const char *find_linux_crt_begin(void)
{
	...
	if (!path)
	{
		...
		// Revert to returning a glob string which we previously did.
		return "/usr/lib/gcc/*/*/";
	}
...
static void linker_setup_linux(const char ***args_ref, Linker linker_type, bool is_dylib)
{
	...
	if (compiler.platform.arch == ARCH_TYPE_RISCV64 || compiler.platform.arch == ARCH_TYPE_RISCV32)
	{
		...
		{
			INFO_LOG("Using RISC-V sysroot at %s", sysroot);
